...
const Catalogue = () => {
  ...
  return (
    <div className="contenu">
      <p className="flex justify-end mb-15 text-sm">
            ...
            <div className="carteBouteille" key={p.id}>
              ...
              <div className="carteContenu">
                <h3 className="font-bold">
                  {p.name} {p.millesime_produit}
                </h3>
                <p>
                  {p.identite_produit} - {p.pays_origine}
                </p>
                ...
                <div className="carteBouton flex justify-between items-center">
                  ...
                  <button
                    className="bg-lime-700 border-transparent hover:bg-lime-800 p-2 rounded-md"
                    id="ajoutBouteille"
                  >
                  ...
                  {/* Ajouter au cellier */}
                  <button
                    className="bg-lime-700 border-transparent p-2 rounded-md hover:bg-lime-800"
                    id="ajoutBouteille"
                  >
                    <Link
                      className="block"
                      to={
                        user && user.id
                          ? `/user/${user.id}/celliers/produits/${p.id}`
                          : "#"
                      }
                    >
                      <svg
      ...
      {totalPages > 1 && (
        <div className="navigationCatalogue div-boutons flex justify-center items-center mb-8 mt-8">
          ...
          {pageCourante !== 1 && (
            <button onClick={() => allALaPage(1)} className="allALaPage">
              1
            </button>
          )}

          {pageCourante > 3 && <span className="text-lg">...</span>}

          {pageCourante > 2 && (
            <button onClick={pagePrecedente} className="pagesuivante">
              {pageCourante - 1}
            </button>
          )}

          <span className="pagerourante">{pageCourante}</span>

          {pageCourante < totalPages - 1 && (
            <button onClick={prochainePage} className="pagesuivante">
              {pageCourante + 1}
            </button>
          )}
          ...
          {pageCourante !== totalPages && (
            <button onClick={() => allALaPage(totalPages)} className="allALaPage">
              {totalPages}
            </button>
          )}
